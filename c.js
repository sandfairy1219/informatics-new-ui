...
btn12.addEventListener('click', function(){ 
    ...
    if (isActive) {
        ...
        const element = document.getElementById('container');
        element.style.display = 'block'
        element.innerHTML = "#include <iostream> <br><br>   main(){<br>   int a,i;  <br>   a=1;                             p.155 실습10 <br>   for(a=1;a<=8;a++)<br>	while(a<8) //while문 쓸때 <br>  {<br>	a++; //while문쓸때 <br>   {<br>       printf(%d의 약수,a);<br>       for(i=1;i<=a;i++)<br>		i = 1; //while문쓸  <br>		while(i<=a){<br>       if(a%i == 0)<br>        printf(%d\n, i);<br>		i++; //while문쓸때 <br>        }<br>       printf(\n);<br>    }<br>       int n, j, i;                           p.156 실습11 <br>      scanf(%d, &n);<br>      for(i=1;i<=n;i++)<br>      {<br>          for(j=1;j<=i;j++)<br>     printf(*);<br>     printf(\n);<br>    }<br>   " 
    }
     else {
